...
class SessionProvisioner
{
    ...
    {
        $vclusterPath = config('govkloud.vcluster.binary_path');
        $kubeconfigPath = config('govkloud.host_k8s.kubeconfig_path');

        // Get kubeconfig from vcluster using --print flag
        // vcluster uses KUBECONFIG env var, not --kubeconfig flag
        $command = sprintf(
            'KUBECONFIG=%s %s connect %s -n %s --print 2>&1',
            escapeshellarg($kubeconfigPath),
            escapeshellarg($vclusterPath),
            escapeshellarg($session->vcluster_release_name),
            escapeshellarg($session->host_namespace)
        );
        ...
        if ($returnCode !== 0) {
            Log::error("Failed to get vcluster kubeconfig", [
                'output' => implode("\n", $output),
            ]);
            return false;
        }
